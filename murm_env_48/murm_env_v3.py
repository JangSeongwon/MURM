...
from roboverse.envs.panda_base_t3 import PandaBaseEnv_t3
...
class MURMENV_v3(PandaBaseEnv_t3):
    ...
    def reset(self, change_object=False):
        ...
        # if self._use_IK:
        self._home_hand_pose = [0.55, 0.0, 1.3,
        ...
        # run_for_goal = self.run_for_goal()
        # checking_IK = self.checkIK()
        #print('IK Calculation', checking_IK)
        # print('Running for Goal', run_for_goal)
    ...
    def run_for_goal(self):
        ...
        p.resetBasePositionAndOrientation(self._obj, goal_pos, q)
        ...
        self.goal_positions = {
             ...
             'panda_joint7': IK[6], 'panda_finger_joint1': 0.015, 'panda_finger_joint2': 0.015,}
    ...
    def checkIK(self):
        ...
        self.goal_positions = {
             'panda_joint1': IK[0], 'panda_joint2': IK[1], 'panda_joint3': IK[2],
             'panda_joint4': IK[3], 'panda_joint5': IK[4], 'panda_joint6': IK[5],
             'panda_joint7': IK[6], 'panda_finger_joint1': 0.04, 'panda_finger_joint2': 0.04,}
        ...
        goal_global = np.uint8(self.render_obs())
        goal_active = np.uint8(self.render_obs_active())
        np.save(image_check_save_path+"v4_1.npy", goal_global)
        np.save(image_check_save_path+"v4_2.npy", goal_active)
    ...
    def get_contextual_diagnostics(self, paths, contexts):
        ...
        for i in range(len(paths)):
            ...
            distance = np.linalg.norm(state[2]-1.017)
...
##############################################################################################
    ...
    def render_obs(self):
        view_matrix_obs = bullet.get_view_matrix(
            target_pos=[0.9, 0, 1.4], distance=0.01, # [0.8, 0, 1.5], distance=0.8,
    ...
    def get_info(self):
        ...
        object_goal_distance = np.linalg.norm(object_pos[2] - 1.017)
        object_pickup = int(object_goal_distance > self._success_threshold)
        info = {'Goal_success': object_pickup}
        return info
    ...
    def my_action(self, goal):
        ...
        on_top = np.linalg.norm(target_pos2[2] - ee_pos[2]) < 0.01
        ready_to_grasp = np.linalg.norm(target_pos - ee_pos) < 0.005
        aligned = np.linalg.norm(target_pos[:2] - ee_pos[:2]) < 0.0075

        done = (np.linalg.norm(target_pos[2] - 1.017) > self._success_threshold) or self.done
