...
def panda_ik(body, link, pos, theta, gripper, gripper_name=None, damping=1e-3,
...
def panda_position_ik(body, link, pos, theta, gripper, gripper_name=None,
...
def panda_position_theta_ik(body, link, pos, theta, gripper, wrist_theta,
                             gripper_name=None, damping=1e-3,
                             gripper_bounds=(-1,1), discrete_gripper=True,
                             max_force=1000.):
    """
    panda_position_ik, but allows for a wrist_theta argument to control
    wrist rotation
    """
    gripper_state = get_gripper_state(body, gripper, gripper_bounds,
                                      discrete_gripper, gripper_name)
...
def step_ik(gripper_range=range(9, 11), body=0):
    '''
...
def _get_gripper_limits(
        body,
        left_gripper_name='panda_finger_joint1',
        right_gripper_name='panda_finger_joint2'
    ):
