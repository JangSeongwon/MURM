...
class MURMENV(PandaBaseEnv):
    ...
    def __init__(self,
                 ...
                 #Image Dimension
                 obs_img_dim=256, #Graduation exam: 1024 #sawyer=48
        ...
        assert DoF in [3, 6, 7]
        # position x,y,z
        # position x,y,z + roll/pitch/yaw of hand frame
        # position x,y,z + quat of hand frame
    ...
    def reset(self, change_object=False):
        ...
        self.goal_pos = np.array([-0.75, 0, 1.4]) #Fixed goal for demo video
...
#################################################################################################
    ...
    def move_obj(self, goal):
        ...
        placing = np.linalg.norm(goal[:2] - target_pos[:2]) < 0.8
        ...
        if grasp:
            ...
            if on_drop_height and not placing:
                ...
                action = np.array([-0.2,-0.45,1.5]) - target_pos
                action[2] = 0
                action *= 0.7
                action = np.append(action, ori_angle)
                self.grip = 1
