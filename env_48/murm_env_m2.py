...
class MURMENV_m2(PandaBaseEnv):
    ...
    def __init__(self,
        ...
        # # _obj POSITION
        self._object_position_low = (0.4, 0, 1.017)
        self._object_position_high = (0.4, 0, 1.017)

        self.default_theta = bullet.deg_to_quat([180, 0, 0])
        self._success_threshold = success_threshold

        self._pos_low = [0.2, -0.5, 1.0]
        self._pos_high = [0.6, 0.1, 1.35]
    ...
    def run_for_goal(self):
        ...
        np.save(image_check_save_path+"v1_1.npy", goal_global)
        np.save(image_check_save_path+"v1_2.npy", goal_active)
        np.save(image_check_save_path+"v1_3.npy", reset_global)
        np.save(image_check_save_path+"v1_4.npy", reset_active)
    ...
    def run_for_goal2(self):
        ...
        final_pos = self.goal_pos + np.array([-0.08, 0, 0.05])
    ...
    def _load_table(self):
        ...
        self._rdbox = bullet.objects.multi_box_goal(
            pos=[np.random.uniform(low=0.3, high=0.5), np.random.uniform(low=-0.4, high=-0.3),
                 np.random.uniform(low=1.02, high=1.2)])
        # self._rdbox = bullet.objects.multi_box_goal(pos=np.array([0.5, -0.4, 1.2]))
        self._objects = {}
        self._sensors = {}
...
##############################################################################################
    ...
    def render_obs(self):
        # Original Global Camera Settings
    ...
    def my_action(self, goal):
        ...
        adjustment1 = np.array([0, 0, 0.05])

        target_pos2 = np.array(bullet.get_body_info(self._obj)['pos'])
        on_drop_height = 0.05 < target_pos[2] - goal[2] < 0.8
        checking_pos = np.array([goal[0] - 0.08, goal[1], goal[2] + 0.05])

        aligned = np.linalg.norm(target_pos - ee_pos) < 0.0075
        ...
        if done and 9 > self.time_add > 5:
            action = np.array([0, 0, 0.4])
            self.grip = -1
            self.achieve_check += 0.33
