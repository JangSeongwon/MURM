...
class VectorQuantizerEMA(nn.Module):
    ...
    def forward(self, inputs):
        # convert inputs from BCHW -> BHWC
        inputs = inputs.permute(0, 2, 3, 1).contigu1ous()
        input_shape = inputs.shape
...
class VQ_VAE(nn.Module):
    def __init__(
        ...
        self.discrete_size = self.root_len * self.root_len #1024
        self.representation_size = self.discrete_size * self.embedding_dim #5120
        # Calculate latent sizes
    ...
    def compute_loss(self, inputs):
        ...
        #print('compute_loss', vq_loss) # Number
        #print('compute_loss', recon) #tensor (IMAGE)
        #print('compute_loss', perplexity) # Number
        #print('compute_loss', recon_error) # Number
    ...
    def encode(self, inputs, cont=True):
        print('Checking VQVAE for PixelCNN')
    ...
    def discrete_to_cont(self, e_indices):
        ...
        #print('e_indices vqvae',e_indices)
    ...
    def decode(self, latents, cont=True):
        if cont:
            # print('emb',self.embedding_dim) #find
...
class CCVAE(nn.Module):
    def __init__(
        ...
        self._encoder = Encoder1(2 * input_channels, num_hiddens,
            num_residual_layers,
            num_residual_hiddens)

        self._cond_encoder = Encoder1(input_channels, num_hiddens,
            num_residual_layers,
            num_residual_hiddens)
        ...
        self._decoder = Decoder1(2 * self.embedding_dim,
            num_hiddens,
            num_residual_layers,
            num_residual_hiddens)

        self._cond_decoder = Decoder1(self.embedding_dim,
            num_hiddens,
            num_residual_layers,
            num_residual_hiddens)
