...
from multiworld.multiworld.core.image_env import ImageEnv
import scipy.misc
import time
import numpy as np
import skvideo.io
...
class RIGVideoSaveFunction:
    ...
    def __init__(self,
                 ...
                 image_obs_key='image_global_observation',
        ...
        self.keys.append('image_global_observation')
        if reconstruction_key:
            print('reconstruction key?')
    ...
    def __call__(self, algo, epoch):
        ...
        # print('video paths = All data', paths)
        self.epoch = epoch
...
#     keys_to_show = keys_to_show or ['image_desired_goal', 'image_global_observation']
#     frames = []
#     N = rows * columns
#     for i in range(N):
#         start = time.time()
...
def reshape_for_video(frames, N, rows, columns, num_channels):
    # print('frame check = 2000', len(frames))
    # print('getting annoyed = 132 132 3' ,frames[0], frames[0].shape)
    ...
    # print('path_length = 400', path_length)
    frames = np.array(frames, dtype=np.uint8).reshape(
        (N, path_length, img_height, img_width, num_channels)
    )
    # print('see frame shape = 400 132 132 3 / 10454000', frames[0].shape, frames.size)
    f1 = []
...
#         return imgs
#     x_0 = path['full_observations'][0]['image_global_observation']
...
def dump_paths(
    ...
    if len(paths) < rows * columns:
        ...
        # print('video column check', len(paths), columns)
    ...
    for i in range(N):
        ...
        path = paths[i]
        ll = []
        print('Check Using Images, Now No Videos')

        for i_in_path, d in enumerate(path['full_observations']):
            # print('i in path 1 to 400', i_in_path)
            # print('d all the infos', d)
            ...
            # print('does ll have imgs == okay')

        frames += ll
        # print('See frames', len(frames))
        ...
        if dirname_to_save_images:
            ...
            # print('rollout_frames', rollout_frames, len(rollout_frames))
            #
